...
author: '500254239 (Diana), 490152629 (Aurora), 490581874 (Sylvia), 490532430 (Vighnesh), 490397493 (Tina), 500459063 (Harun), 500505692 (David)'
...
knitr::opts_chunk$set(
  echo = FALSE,    # for final report, set to FALSE to hide all code chunks
  message = FALSE,
  warning = FALSE,
  fig.align = "center",
  fig.cap = "",
...
The residual standard error (RSE) was used to assess the fit of the models to the people vaccinated attribute. The mean RSE over all the locations is $`r round(mean(unlist(r_list1["rse"]), na.rm = TRUE), 2)`$ for the logistic model, which is lower than $`r round(mean(unlist(r_list2["rse"]), na.rm = TRUE), 2)`$ for the asymptotic regression. (Figure \@ref(fig:rse-boxplot)) Therefore, the logistic model was the most appropriate method for estimating the vaccination uptake parameter.
...
## Regression model goodness-of-fit (for $r$ estimation)

```{r rse-boxplot, fig.cap="RSE comparison of logistic vs asymptotic model."}
boxplot(unlist(r_list1$rse), unlist(r_list2$rse), names = c("Logistic Growth Model", "Asymptotic Regression"), main = "Residual Standard Errors of Fitted Models", xlab = "Model", ylab = "Log Residual Standard Error (RSE)", log = "y")
```
