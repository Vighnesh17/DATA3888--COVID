...
for (loc in unique(covid_clean$location)) {
  ...
  fit = tryCatch(
    ...
    error = function(error_message){
      return( lm(people_vaccinated ~ t_days, data = covid_subset) )
    }
  )
...
ggplot(df, aes(x=vri, y= log(extreme_poverty))) + geom_point() + ggtitle("Extreme Poverty vs Vaccine Roll-out Index(vri)") #
...
set.seed(1)
optimal_rf <- randomForest(vri ~ ., data= scaled_data, mtry = 3, replace = FALSE, 
...
rfProfile <- rfe((scaled_data %>% select(-vri,-human_development_index)), as.matrix(scaled_data %>% select(vri)),
                 sizes = subsets,
                 rfeControl = ctrl)


result <- predict(rfProfile,newdata=scaled_data %>% select(-vri))
result_avg <- mean(scaled_data$vri)
r2 = 1 - (sum((scaled_data$vri - result)^2))/(sum((scaled_data$vri - result_avg)^2))
...
```{r}
set.seed(1)
optimal_rf <- randomForest(vri ~ ., data= scaled_data, mtry = 4, 
                              keep.forest = TRUE, ntree= 300)
## compute permutation importance
rf_permimp <- permimp(optimal_rf, progressBar = FALSE, conditional = TRUE,scaled = TRUE,thresholdDiagnostics = TRUE)
## barplot
plot(rf_permimp)
