...
get_slope <- function(country_data, country_policy, country) {
  ...
  while (i <= length(country_policy$vaccination_policy)) {
    ...
    if (current_policy > 0 & !is.na(country_policy$people_vaccinated[country_policy$vaccination_policy == current_policy])) {
      ...
      temp_people <- country_data$people_vaccinated[country_data$vaccination_policy == current_policy & !is.na(country_data$people_vaccinated)]
...
```{r}
get_slope(country_data, country_policy, "Brazil")
...
```{r}
ggplot(data = country_slope[country_slope$country %in% c("United States", "India", "Brazil", "France", "United Kingdom", "Russia", 
              "Turkey", "Italy", "Germany"), ], aes(x = country, y = speed, fill = as.factor(policy))) + 
  geom_bar(stat="identity", position=position_dodge()) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  scale_y_sqrt() + 
  ggtitle("United States")
