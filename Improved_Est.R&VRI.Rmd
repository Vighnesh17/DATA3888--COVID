...
```{r}
availability <- read.csv("/Users/davidvucic/Desktop/vaccination_policy.csv")
availability$policy <- ordered(availability$policy, levels = c(0,1,2,3,4,5))
...
vri_data <- merge(vri_data, availability, by.x = "location", by.y = "location")
...
temp_data <- data.frame(lapply(vri_data[3:8], min_max_norm), availability = vri_data$policy, vri = vri_data$vri)
