ct_model = function(df, log.y = FALSE, model = c("logis", "asymp", "linear"), model2 = "none", scal.start = 4, lrc.start = -5, logisOffset = 0, asympOffset = 0.1) {
  # cleaning
  covid_clean = df %>% 
      ...
      fit = tryCatch(
        ...
        error = function(error_message){
          ...
          if (model2 == "linear") {
            ...
            y = if(log.y) log(covid_subset$people_vaccinated) else covid_subset$people_vaccinated
            Asym0 = max(y)
            halfy.point = which.min(abs(y - 1/2*max(y)))
            xmid0 = covid_subset$t_days[halfy.point]
            scal0 = scal.start    # chosen based on manual trial and error on some failed SSlogis cases
            return( tryCatch(
              expr = nls(f3, data = covid_subset, start = list(Asym = Asym0, xmid = xmid0, scal = scal0), control = list(maxiter = 100, minFactor = 1/2048, warnOnly = TRUE, scaleOffset = logisOffset)),
            ...
            Asym0 = max(y)
            R00 = min(y)
            lrc0 = lrc.start    # chosen based on manual trial and error on some failed SSasymp cases
            return( tryCatch(
              expr = nls(f3, data = covid_subset, start = list(Asym = Asym0, R0 = R00, lrc = lrc0), control = list(maxiter = 100, minFactor = 1/2048, warnOnly = TRUE, scaleOffset = asympOffset)),
              # if still fails, then return NULL (most likely not enough points)
              error = function(e) { return( NULL ) }
            ) )
          }
